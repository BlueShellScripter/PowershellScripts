...
# This script pulls all users in targetMailboxes.txt (username format, one per line)
...
# Connect to Exchange
write-host "Ensure your account is PIM'd or has sufficient permissions before logging in."
...
# all staff is :
Write-Host "Pulling usernames from targetMailboxes.txt"
$usernames = Get-Content "targetMailboxes.txt"

write-host "$($usernames.count) Accounts have been collected.`n`n"
...
$usernames.foreach({
    ...
    if (Get-ADUser -Filter "Name -eq '$_'"){
        ...
        #get user info
        $uniqueID = Get-ADUser -Filter "Name -eq '$_'" -Properties UserPrincipalName, EmailAddress | Select-Object -Property UserPrincipalName, EmailAddress -ErrorAction Stop
        ...
        try {
            ...
            if ($rootPermissions.AccessRights -ne "None" ) {
                $usersWithRootPermissions += $_ 
                # method to flag as asterix on a finding, a red asterix will show up in the 'loading bar' of .'s from above
                write-host "*" -nonewline -foregroundcolor Red 
            }
            
            # Get the Inbox folder statistics for the user, instead of looking for 'inbox' which will error on non english mailboxes
            $inboxStats = Get-MailboxFolderStatistics -Identity $uniqueID.UserPrincipalName | Where-Object {$_.FolderType -eq "Inbox"} -ErrorAction Stop -verbose -debug
            ...
            if ($inboxStats) {
                ...
                $folderIdentity = "$($_):\${inboxFolderId}"
            ...
            #mailbox not found at all, write an x and skip
...
if ($usersWithRootPermissions -ne @()){
    ...
    $usersWithRootPermissions | ForEach-Object { Write-Host $_ }
    ...
    # built in fix for root permissions:
...
if ($usersWithInboxPermissions -ne @()){
    ...
    # Iterate through each user and print their name
    $usersWithInboxPermissions | ForEach-Object { Write-Host $_ }
    # Convert each user to a custom object and export to CSV
    ...
    # fix for inboxes not implemented using new method, current method below only fixes inboxes named 'inbox'
    <# currently this will only work for those with english inboxes -- haven't had a need to change it personally
    $response = Read-Host "Would you like to change the permissions back to 'None' for these users? (y/n)"
    if ($response -eq 'y') {
...
if ($usersWithoutInbox -ne @()) {
    ...
    # Iterate through each user and print their name
    $usersWithoutInbox | ForEach-Object { Write-Host $_ }
    # Convert each user to a custom object and export to CSV
